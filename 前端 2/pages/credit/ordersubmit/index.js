...
var points_rate;
Page({
  data: { use_money: 0, express_price: 0, use_point: 0, check: ['true', ''], "coupon": [], cv: '请选择优惠劵', cpos: -1, "couponCode": '', "pay_type_list": '', "payment": 'weixin', 'goods_spec':0 },
  ...
  onShow: function () {
    var app = getApp();
    this.getCarts(this.data.goods_id, this.data.goods_num, this.data.goods_spec);
  ...
  formSubmit: function (e) {
    ...
    if (this.data.payment == 'balance') {
      wx.showModal({
        title: '提示',
        content: '是否使用余额支付',
        success: function (res) {
          if (res.confirm) {
            that.submit_order(user_id, address_id, use_money, pay_points, couponTypeSelect, coupon_id, pay_code, couponCode);
          } else {
            return;
          }
        }
      ...
      that.submit_order(user_id, address_id, use_money, pay_points, couponTypeSelect, coupon_id, pay_code, couponCode, that.data.goods_id, that.data.goods_num, that.data.goods_spec, that.data.express_price, that.data.totalJifen);

    }
  },

  submit_order: function (user_id, address_id, use_money, pay_points, couponTypeSelect, coupon_id, pay_code, couponCode, goods_id, goods_num, goods_spec, shippingPrice, totalJifen) {
    var app = getApp();
    var that = this;
    if (!goods_spec){
      goods_spec = 0;
    }
    server.getJSON('/jifen/addOrder/user_id/' + user_id + "/address_id/" + address_id + "/user_money/" + use_money + "/pay_points/" + pay_points + "/couponTypeSelect/" + couponTypeSelect + "/coupon_id/" + coupon_id + "/pay_code/" + pay_code + '/goods_id/' + that.data.goods_id + '/goods_num/' + goods_num + '/shippingPrice/' + shippingPrice + '/totalJifen/' + totalJifen + '/goods_spec/' + goods_spec + "/couponCode/" + couponCode, function (res) {
  ...
  getCarts: function (goods_id, goods_num, goods_spec) {
    var user_id = getApp().globalData.userInfo.user_id
    var that = this
    var app = getApp()

    server.getJSON('/jifen/cart/user_id/' + user_id + '/goods_id/' + goods_id + '/goods_num/' + goods_num + '/goods_spec/' + goods_spec, function (res) {
      ...
      if (res.data.result) {
        ...
        var totalJifen = res.data.result.totalJifen
        tp = totalPrice.total_fee
        points_rate = res.data.result.points
        that.setData({ address: address, cartList: cartList, userInfo: userInfo, totalPrice: totalPrice, express_price: express_price, pay_type_list: pay_type_list, totalJifen: totalJifen});
        that.initData();
      } else {
        wx.redirectTo({
          url: "../../member/index/index"
