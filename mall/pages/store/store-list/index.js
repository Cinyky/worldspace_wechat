...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    that.setData({
      p_cate_select_id: options.cate_id ? options.cate_id:-1,
      select_cate_id: options.cate_id ? options.cate_id:'',
      c_cate_select_title: options.cate_name ? options.cate_name :'全部'
    });
  ...
  clickCate: function (e) {
    ...
    if (type == 1) {
      ...
      //当前选择 如果没有二级分类 则直接搜索当前一级分类
      if(index>0){
        var flag = that.data.cates[index];
        if (flag.c_cates.length <= 0) {
          that.setData({
            c_cate_select_id: id,          
            is_show_cates: false,
            c_cate_select_title: flag['title'],
            select_cate_id: id
          });
          that.onPullDownRefresh();
        }
