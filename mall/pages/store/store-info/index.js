...
Page({
  data: {
    ...
    duoguan_app_index_path: _DgData.duoguan_app_index_path,
    duoguan_app_is_superhome: _DgData.duoguan_app_is_superhome,
  },
  // 轮播图渲染数字
  ...
  backHome: function () {
    var that = this;
    if (that.data.duoguan_app_is_superhome){
      _dg.switchTab({
        url: that.data.duoguan_app_index_path
      });
    }else{
      _dg.switchTab({
