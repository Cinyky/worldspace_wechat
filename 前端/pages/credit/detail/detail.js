...
Page({
  ...
  bug: function () {
    ...
    // server.getJSON('/Cart/addCart', { goods_id: goods_id, goods_spec: goods_spec, session_id: session_id, goods_num: goods_num, user_id: user_id }, function (res) {
      ...
      //   show_attr_picker: !1
      // })
    // console.log(that.data.goods.goods.store_count);
    if (goods_num > that.data.goods.goods.store_count) {
      wx.showToast({
          title: '库存不足',
          icon: 'error',
          duration: 2000
      ...
      return;
        ...
        wx.navigateTo({
          // url: '../ordersubmit/index?cartIds=' + goods_id + '&amount=' + goods_num * goods.goods.shop_price
          url: '../ordersubmit/index?goods_id=' + goods_id + '&goods_spec=' + goods_spec + '&goods_num=' + goods_num
        });
      // } else {
      //   wx.showToast({
      //     title: res.data.msg,
