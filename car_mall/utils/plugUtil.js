...
function onNavigateTap(e) {
    ...
    let url = dataset.url; url = url ? url + (url.indexOf('?') != -1 ? '&' : '?') + "module=" + dataset.module : "";
    const type = dataset.type, nav = { url: url };
...
export default class PlugUtils {
    ...
    static popup(pageObj, moduleName = '', name = "ad_popup") {
        ...
        requestUtil.get(url, { module_name: moduleName }, (data) => {
            ...
            if (orgPopupData && orgPopupData.length && orgPopupData[0].min) {
                _.each(data, item => { item.min = true; item.module = moduleName; });
                pageObj.setData(makeObjectData(name, data));
            } else {
                _.each(data, item => { item.module = moduleName; });
                pageObj.setData(makeObjectData(name, data));
                timerId = setTimeout(() => {
     ...
     */
    static share() {
        const url = API_HOST + '/index.php/addon/DuoguanUser/ApiMarketing/shareSetData';
     ...
     * 分享接口
     */
    static setShare(pageObj, moduleName, pagePath) {
        pagePath = pagePath ? pagePath : pageObj.route;
        wx.hideShareMenu();
        const url = API_HOST + '/index.php/addon/DuoguanUser/Api/getAddonIndexPage';
        requestUtil.get(url, { modulename: moduleName }, (info) => {
            if (!info) info = getCurrentPages()[0].route;
            
            pageObj.onShareAppMessage = function () {
                const title = this.title || '';
                const url = info + (info.indexOf('?') !== -1 ? '&' : '?') + '_path=' + encodeURIComponent('/' + pagePath);
                return {
